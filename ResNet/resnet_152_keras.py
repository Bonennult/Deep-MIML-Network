...
#from keras.layers import KL.Input #, KL.Dense, KL.MaxPooling2D, KL.AveragePooling2D, Dropout, KL.Flatten, merge, Reshape, KL.Activation, Lambda, GlobalKL.AveragePooling2D
...
class Scale(Layer):
    '''Custom Layer for ResNet used for KL.BatchNormalization.
...
def resnet152_model(weights_path=None):
    ...
    if K.image_dim_ordering() == 'tf':
        bn_axis = 3
        img_input = KL.Input(shape=(224, 224, 3), name='data')
    else:
        bn_axis = 1
        img_input = KL.Input(shape=(3, 224, 224), name='data')
