...
from util.feat_extractor import load_model, get_CAM, feat_pred, feat_pred_by_seg
...
def preprocess_for_test_by_seg(test_path):
    # test_path = 'h:/Study/bpfile/testset25/'
    wav_files = []
    img_files = []
    bases_filepath = []
    ...
    for wav_fname, img_folder in zip(wav_files, img_files):
        ...
        # 计算图像的 label，并获得每种乐器的定位（列号）
        for img in imgs:
            left,right = feat_pred_by_seg(img_dir+img_folder, img)
